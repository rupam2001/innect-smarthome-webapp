...
import { CircleRounded, PlayArrow, Settings } from "@mui/icons-material";
...
export default function DevicesScreen() {
  ...
  const getRoomDataAsync = async () => {
    return new Promise((resolve) =>
      resolve(globalContext.rooms?.find((v) => v._id == roomId))
    );
  };
  ...
  const handleSwitchToggle = (e, device, s) => {
    ...
    sendMessage(
      JSON.stringify({ is_command: true, device_id: device._id, command: s.id })
    );
  };
  ...
  useEffect(() => {
    ...
    sendMessage(
      JSON.stringify({
        is_command: true,
        device_id: roomData.devices.map((d) => d._id),
        command: "",
      })
    );
  }, [roomData]);

  const handleOnClickSettings = (device) => {
    const { _id } = device;
    console.clear();
    console.log(device);
    navigate({
      pathname: "/wifi",
      search: createSearchParams({ device_id: _id }).toString(),
    });
  };
    ...
    <InAppLayout>
      <div className="m-2 text-green-500">
        <Breadcrumbs aria-label="breadcrumb">
          <Link to={"/rooms"}>
            <span className="text-green-500">{roomTitle}</span>
          </Link>
            ...
            <div key={device._id}>
              <div className="my-2 flex items-center">
                <div className="font-bold">
                  <span>Device {getDeviceIDForUI(device)}:</span>
                </div>
                <div className="mx-2 font-bold flex-1">
                  {getDeviceStatus(device) && (
                    <CircleRounded
                      style={{ fontSize: "small", color: "#22c55e" }}
                    />
                  )}
                  {!getDeviceStatus(device) && (
                    <CircleRounded
                      style={{ fontSize: "small", color: "#e11d48" }}
                    />
                  )}
                </div>
                <div
                  className="mr-4"
                  onClick={() => handleOnClickSettings(device)}
                >
                  <Settings style={{}} />
                </div>
              </div>
              ...
              <div className="m-2 ml-4">
                ...
                {device.switches.map((s) => {
                  if (
                    ...
                    return (
                      <div
                        className="flex w-full justify-between my-4 items-center"
                        key={s._id}
                      >
                        <div className="text-xl text-sky-400 font-bold">
                          {s.title}
                        </div>
                        <IOSSwitch
                          sx={{ m: 1 }}
                          checked={getSwitchState(device, s)}
                          onChange={(e) => handleSwitchToggle(e, device, s)}
                        />
                      </div>
                    );
                  } else {
